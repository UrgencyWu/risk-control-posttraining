...
| **C7** | **SFT multi (German)** | **0.720** | **142** | **多数据集负迁移** |
...
**产出**: `src/evaluation/c7_final.py`，`outputs/c7_final_metrics.json`，`docs/EVALUATION_PROTOCOL.md`

**评测口径（修订后）**: C7 只读取版本控制的 valid/test 预测工件；阈值仅由 valid 集按 Cost=5FN+FP 选择，然后冻结并应用于 test 集。C7 不加载模型、不重跑推理，也不使用 test 标签搜索 operating point。
...
| SFT multi (German) | 0.720 | 0.527 | 0.568 | 0.194 | 0.059 | 142 | 0.769 | 0.504 |
...
**多数据集 SFT**: 加入 Australian 后 German ROC-AUC 从 0.747 降至 0.720，说明多数据集训练提升了跨数据集覆盖但未增强 German 单任务效果。
