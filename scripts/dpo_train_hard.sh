#!/bin/bash
#SBATCH --job-name=dpo_hard
#SBATCH --partition=compute
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --gres=gpu:2
#SBATCH --time=24:00:00
#SBATCH --output=logs/dpo_hard_%j.out
#SBATCH --error=logs/dpo_hard_%j.err

source /home/wushaohua/miniconda3/etc/profile.d/conda.sh
conda activate qwen9B

echo "=== C5v2: DPO (Hard Preference) ==="
...
ADAPTER_DIR=outputs/dpo/german_dpo_seed42_hard
PREF_DATA=data/processed/german/preference/preference_train_hard.jsonl

echo "--- Training ---"
python3 src/training/dpo_train.py --mode dpo --pref_data "$PREF_DATA" --seed 42 --tag hard
